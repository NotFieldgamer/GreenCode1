...
const globalStyles = `
  ...
  .nav-bar { padding: 1rem 3rem; }
  .hero-section { padding: 6rem 3rem 3rem; flex-direction: row; }
  .hero-content { max-width: 600px; padding-top: 4rem; text-align: left; }
  .hero-3d { position: absolute; right: 0; top: 0; bottom: 0; width: 55%; }
  .hero-buttons { display: flex; gap: 1rem; flex-direction: row; }
  .stats-container { gap: 5rem; }
  .features-container { padding: 8rem 3rem; }
  ...
  @media (max-width: 992px) {
    ...
    .hero-3d { 
      position: relative; 
      width: 100%; 
      height: 400px; /* Gives the 3D canvas room below the text */
      margin-top: 2rem;
    }
    .nav-bar { padding: 1rem 1.5rem; }
    .stats-container { gap: 2rem; }
    .features-container { padding: 4rem 1.5rem; }
  }
...
export default function Landing() {
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', overflowX: 'hidden', width: '100vw' }}>
        ...
        <nav className="nav-bar" style={{
          ...
          <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', fontWeight: 700, fontSize: '1.2rem', color: '#fff', textDecoration: 'none' }}>
            <div style={{ width: 32, height: 32, background: 'var(--green)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 15px rgba(0, 255, 204, 0.4)', flexShrink: 0 }}>
              <TrendingUp size={18} color="#000" />
            </div>
            GreenCode
          </Link>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <Link to="/leaderboard" className="hide-on-mobile" style={{ color: 'var(--text-muted)', fontSize: '0.9rem', textDecoration: 'none', transition: 'color 0.2s' }}>Leaderboard</Link>
            <Link to="/pricing" className="hide-on-mobile" style={{ color: 'var(--text-muted)', fontSize: '0.9rem', textDecoration: 'none', transition: 'color 0.2s' }}>Pricing</Link>
            
            {/* Guaranteed Visible Buttons */}
            {isAuthenticated ? (
              <Link to="/dashboard" style={{
                background: 'var(--green)', color: '#000', padding: '0.5rem 1.25rem', borderRadius: '8px', fontWeight: 600, textDecoration: 'none', boxShadow: '0 4px 14px rgba(0,255,204,0.3)', whiteSpace: 'nowrap'
              }}>Dashboard</Link>
            ) : (
              <div style={{ display: 'flex', gap: '0.75rem', marginLeft: 'auto' }}>
                <Link to="/login" className="hide-on-mobile" style={{
                  color: '#fff', padding: '0.5rem 1.25rem', borderRadius: '8px', fontWeight: 500, textDecoration: 'none', border: '1px solid var(--glass-border)', background: 'var(--glass-bg)', whiteSpace: 'nowrap'
                }}>Sign In</Link>
                <Link to="/register" style={{
                  background: 'var(--cyan)', color: '#000', padding: '0.5rem 1.25rem', borderRadius: '8px', fontWeight: 600, textDecoration: 'none', boxShadow: '0 4px 14px rgba(0, 212, 255, 0.3)', whiteSpace: 'nowrap'
                }}>Get Started</Link>
              </div>
            )}
          </div>
        ...
        <section className="hero-section" style={{
          position: 'relative', minHeight: '100vh', display: 'flex', boxSizing: 'border-box', width: '100%'
        }}>
          ...
          {/* Content Area */}
          <div className="hero-content" style={{ flex: 1, position: 'relative' }}>
            <div style={{
            ...
            {/* Adjusted typography minimum clamp size so it fits on mobile */}
            <h1 style={{ fontSize: 'clamp(2.2rem, 6vw, 4.5rem)', fontWeight: 700, lineHeight: 1.1, letterSpacing: '-0.03em', marginBottom: '1.5rem', textShadow: '0 4px 20px rgba(0,0,0,0.5)' }}>
            ...
            <div style={{ opacity: 0, animation: 'fadeSlideUp 0.6s ease 1.1s forwards' }}>
              <p style={{ fontSize: 'clamp(1rem, 2vw, 1.15rem)', color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: '2.5rem' }}>
                GreenCode uses AI to construct an interactive map of your software's energy consumption. Spot memory leaks, visualize algorithmic complexity, and neutralize your carbon footprint.
              </p>
            </div>

            {/* Hero Buttons */}
            <div className="hero-buttons" style={{ opacity: 0, animation: 'fadeSlideUp 0.6s ease 1.3s forwards' }}>
              <Link to="/register" style={{
                display: 'inline-flex', alignItems: 'center', gap: '0.5rem',
          ...
          <div className="hero-3d" style={{ zIndex: 5, pointerEvents: 'auto' }}>
        ...
        {/* ── Stats Strip ────────────────────────────────────────────── */}
        <section style={{ position: 'relative', zIndex: 10, background: 'rgba(5, 5, 15, 0.8)', backdropFilter: 'blur(20px)', borderTop: '1px solid var(--glass-border)', borderBottom: '1px solid var(--glass-border)', padding: '2.5rem 1rem' }}>
          <div className="stats-container" style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', maxWidth: '1200px', margin: '0 auto' }}>
            {STATS.map(s => (
              <div key={s.label} style={{ textAlign: 'center', minWidth: '120px' }}>
                <div style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: 'clamp(1.8rem, 4vw, 2.2rem)', color: s.color, textShadow: `0 0 15px ${s.color}40` }}>{s.value}</div>
                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '0.5rem', textTransform: 'uppercase', letterSpacing: '1px' }}>{s.label}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* ── Features Grid ──────────────────────────────────────────── */}
        <section className="features-container" style={{ maxWidth: '1300px', margin: '0 auto', position: 'relative', zIndex: 10 }}>
          <FadeIn>
            <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: 'clamp(1rem, 2vw, 1.1rem)', maxWidth: '600px', margin: '0 auto', lineHeight: 1.5 }}>
            ...
            {FEATURES.map((f, i) => (
              <FadeIn key={f.title} delay={i * 0.1}>
                ...
                onMouseLeave={e => {
                  ...
                  <div style={{ width: '45px', height: '45px', borderRadius: '12px', background: `${f.color}15`, color: f.color, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.5rem', border: `1px solid ${f.color}30` }}>
                    {f.icon}
                  </div>
                  <h3 style={{ fontWeight: 600, fontSize: '1.2rem', marginBottom: '0.75rem', color: '#fff' }}>{f.title}</h3>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', lineHeight: 1.6, margin: 0 }}>{f.desc}</p>
                </div>
              </FadeIn>
            ))}
          </div>
        ...
        {/* ── Footer ─────────────────────────────────────────────────── */}
        <footer style={{ borderTop: '1px solid var(--glass-border)', background: '#030308', padding: '2.5rem 1.5rem', position: 'relative', zIndex: 10 }}>
          <div className="footer-inner" style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: '#fff', fontWeight: 600, justifyContent: 'center', width: '100%', flex: 1, minWidth: '200px' }}>
              ...
              GreenCode
              <span style={{ color: 'var(--text-muted)', fontWeight: 400, marginLeft: '0.5rem', fontSize: '0.85rem' }}>© 2026 Sustainable Tech</span>
            </div>
            
            <div style={{ display: 'flex', gap: '1.5rem', justifyContent: 'center', width: '100%', flex: 1, flexWrap: 'wrap' }}>
              <Link to="/leaderboard" style={{ color: 'var(--text-muted)', textDecoration: 'none', transition: 'color 0.2s', fontSize: '0.9rem' }} onMouseEnter={e => e.target.style.color = '#fff'} onMouseLeave={e => e.target.style.color = 'var(--text-muted)'}>Leaderboard</Link>
              <Link to="/pricing" style={{ color: 'var(--text-muted)', textDecoration: 'none', transition: 'color 0.2s', fontSize: '0.9rem' }} onMouseEnter={e => e.target.style.color = '#fff'} onMouseLeave={e => e.target.style.color = 'var(--text-muted)'}>Pricing</Link>
              <Link to="/login" style={{ color: 'var(--text-muted)', textDecoration: 'none', transition: 'color 0.2s', fontSize: '0.9rem' }} onMouseEnter={e => e.target.style.color = '#fff'} onMouseLeave={e => e.target.style.color = 'var(--text-muted)'}>Sign In</Link>
            </div>
          </div>
        </footer>
      </div>
